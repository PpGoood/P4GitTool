...
export function createDiscardRouter(rootDir: string): Router {
  const router = Router();
  ...
  router.post('/discard-file', async (req, res) => {
    ...
    try {
      const ok = await ops.discardFile(rootDir, stream, path, makeLogFn());
  ...
  router.post('/discard-hunk', async (req, res) => {
    ...
    try {
      const ok = await ops.discardHunk(rootDir, stream, path, parseInt(hunkIndex, 10), makeLogFn());
  ...
  router.post('/discard-line', async (req, res) => {
    ...
    try {
      const ok = await ops.discardLine(rootDir, stream, path, parseInt(hunkIndex, 10), parseInt(lineIndex, 10), makeLogFn());
