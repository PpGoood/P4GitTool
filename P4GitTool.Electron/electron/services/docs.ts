...
| 文件夹 | 放什么 | 内部结构 |
|--------|--------|----------|
| \`tech-design/{分支}/{功能}/\` | 功能开发方案、协议、配表、策划原文 | 按功能分文件夹，每个功能夹含一份 \`上下文.md\` |
| \`bugs/{分支}/{功能}/\` | 问题分析、根因排查、修复方案 | 按功能分文件夹，每个功能夹含一份 \`上下文.md\` |
...
- 每个功能文件夹里放一份 \`上下文.md\`：记录做这个功能 / 修这个 bug 的对话记忆，方便多会话接续
- 功能名不存在就新建文件夹
...
- 功能文件夹内的文档：\`主题.md\` 即可（已在功能夹下，不必再带功能名前缀），如 \`制造系统/架构总结.md\`、\`制造系统/任务进度不刷新-修复.md\`
- 每个功能夹的对话记忆固定叫 \`上下文.md\`
- \`knowledge/\` 下的稳定规范用简洁主题名，如 \`unlua导出规则.md\`
...
| 文件夹 | 放什么 |
|--------|--------|
| \`tech-design/{分支}/{功能}/\` | 该功能的技术方案、协议、配表、策划原文 + 一份 \`上下文.md\` |
| \`bugs/{分支}/{功能}/\` | 该功能的 bug 分析、根因、修复 + 一份 \`上下文.md\` |
| \`knowledge/{工作流|引擎|项目功能}/\` | 通用规范、工具指南、引擎知识 |
| \`attachments/\` | 图片、截图 |

- 同一功能在 tech-design 和 bugs 用**同名文件夹**（如都叫 \`制造系统\`）
- 每个功能文件夹放一份 \`上下文.md\`：做这个功能 / 修这个 bug 的对话记忆
...
- 一个功能一个文件夹（如 \`dev/制造系统/\`），夹内含该功能的文档 + 一份 \`上下文.md\`
- frontmatter 类型: 技术方案
`,
  'bugs': `# bugs · Bug 分析与修复
...
- 按分支分 \`dev/\`、\`release/\`，**分支下一律按功能建子文件夹**，不准平铺
- 功能文件夹和 tech-design 同名（如都叫 \`制造系统\`），夹内含 bug 文档 + 一份 \`上下文.md\`
- frontmatter 类型: bug
`,
